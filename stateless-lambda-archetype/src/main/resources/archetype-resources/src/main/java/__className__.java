...
public class ${className} implements RequestHandler<List<HashMap<String, String>>, String> {
    ...
    private void createAlert(String evt, String dt_id) {
        ...
        item.put("id", new AttributeValue(dt_id));
        item.put("rt", new AttributeValue(lambdaTimeStr));
        item.put("events", new AttributeValue(evt));

        // Create the PutItemRequest
        PutItemRequest putItemRequest = new PutItemRequest()
            .withTableName("AlertDB")
            .withItem(item);

        try {
            // Execute the request
            PutItemResult result = client.putItem(putItemRequest);
            logger.log("Alert created. Event: " + evt);
        } catch (Exception e) {
            logger.log("Unable to add item: " + dt_id 
              + " - Evt: " + evt + " Exception: "+e.getMessage());
        }
    }
